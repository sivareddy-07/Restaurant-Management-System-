...
public class DataSeeder implements CommandLineRunner {
    ...
    public DataSeeder(MenuItemRepository menuItemRepository, RestaurantTableRepository tableRepository, CustomerOrderRepository orderRepository) {
    ...
    public void run(String... args) throws Exception {
        if (menuItemRepository.count() == 0) {
            menuItemRepository.save(new MenuItem("Bruschetta", "Toasted bread with tomatoes/basil", new BigDecimal("8.50"), "Starter"));
            menuItemRepository.save(new MenuItem("Grilled Salmon", "Fresh salmon with sides", new BigDecimal("24.00"), "Main"));
            menuItemRepository.save(new MenuItem("Steak Frites", "Ribeye with fries", new BigDecimal("32.50"), "Main"));
            menuItemRepository.save(new MenuItem("Tiramisu", "Classic espresso dessert", new BigDecimal("9.00"), "Dessert"));
        ...
        if (orderRepository.count() == 0) {
            RestaurantTable t2 = tableRepository.findAll().stream().filter(t -> t.getTableNumber().equals("T2")).findFirst().orElse(null);
            if (t2 != null) {
                ...
                MenuItem salmon = menuItemRepository.findAll().stream().filter(m -> m.getName().equals("Grilled Salmon")).findFirst().orElse(null);
                MenuItem tea = menuItemRepository.findAll().stream().filter(m -> m.getName().equals("Iced Tea")).findFirst().orElse(null);
